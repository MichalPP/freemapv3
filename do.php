...
foreach($config as $k => $v) if(1) { // ak je plugin spomenuty v html
	...
	$minzoom = 13; if($v['onmove_minzoom'] > 1) $minzoom= $v['onmove_minzoom'];
	if(strlen($v['onmove']) >5) {if(strlen($replace['onmove'])>2) $replace['onmove'] .= "else "; $replace['onmove'] .= "if(activediv == '#$k' && mapa.getZoom() >= $minzoom) {".$v['onmove']."}\n"; }
	$replace['plugins-jquery'] .= $v['jquery'];
	$replace['plugins-css'] .= $v['css'];
	$replace['plugins-css-mobile'] .= $v['css-mobile'];
	if(strlen($v['menu-horne']) >2) $replace['menu-horne'] .= "<a href='#$k'>".$v['menu-horne']."</a>";
}
$replace['onmove'] .= "else { routeLayer.clearLayers(); $('.zoznam').text(' ');}\n";
...
if($runfull) {
	...
	system('cat tmp/css.css | php embedimg.php|yui-compressor --type css > tmp/freemap-css.css');
} else { system("cat tmp/lib-js/9* > tmp/freemap-js.js"); system('cat tmp/css.css | php embedimg.php > tmp/freemap-css.css'); }
