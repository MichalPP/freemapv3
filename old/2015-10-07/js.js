...
if(typeof a['map'] === 'string') {
    loc=a['map'].split('/');
//    mapa.setZoom(loc[1]).panTo([loc[2], loc[3]]);
//    mapa.removeLayer(layers.getActiveBaseLayer().layer); mapa.addLayer(window[loc[0]]);
}
...
$("#vrstvy .vrstva").click(function(e) {
	e.preventDefault();
	//$("label:has(span:contains("+$(this).html()+"))").children("input").click(); toright(); return false; 
	//if(layers.getActiveBaseLayer().name == $(this).html()) {return false; }
	if(typeof baseLayers[$(this).html()] === 'object') { 
		...
		//mapa.removeLayer(layers.getActiveBaseLayer().layer); mapa.addLayer(baseLayers[$(this).html()]); 
        //console.log(layers.getActiveBaseLayer().name);
		hreflink(activediv); 
		toright();
		}
