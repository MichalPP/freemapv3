...
function resizebigger() {
	...
	if( $(window).width() < 520) {
		...
		$("#leftpart, #leftmenu, #topsmallmenu").css('max-height', (window.innerHeight - $("#topmenu").height() - $(".reklamaall").height())+'px').css('height','');
	} else {
		...
		$("#leftpart").height(window.innerHeight - $("#topmenu").height()- $("#leftmenu").height() - $(".reklamaall").height());
	}
	$(".fullh").height(window.innerHeight - $("#topmenu").height());
	mapa.invalidateSize();
}
resizebigger();
...
function zoznamclick(feature) {
	...
	else { id=feature.id; document.getElementById(id).scrollIntoView(); }
