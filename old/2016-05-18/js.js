...
function divshow(tt) {
	...
	activediv=tt;
	if($(tt).length!=1 ) {
        $("#leftpart").prepend("<div id='"+tt.replace('#', '')+"' class='submenu'></div>");
...
function resizebigger() {
	...
	whei = Math.min(window.screen.availHeight, window.innerHeight);
	if( $(window).width() < 520) {
		...
		$("#leftpart, #leftmenu, #topsmallmenu").height(whei - $("#topmenu").height() - $(".reklamaall").height()); //.css('height','');
	} else {
		...
		$("#leftpart").height(whei - $("#topmenu").height()- $("#leftmenu").height() - $(".reklamaall").height());
	}
	$(".fullh").height(whei - $("#topmenu").height());
...
function zoznamclickpart(id) {
    ...
    routeLayer.eachLayer(function (layer) {
        ...
        if(layer.feature.properties.id==id || layer.feature.id==id) {layer.setStyle({opacity: 1, weight: 25 });}
    });
}
...
var routeLayer = L.geoJson({ type: 'FeatureCollection', features: [] }, {
        ...
        onEachFeature: function (feature, layer) { 
            layer.on('click', function () { zoznamclick(feature); });
		},
