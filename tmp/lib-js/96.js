...
function divshow(tt) {
	...
	if( $(window).width() > 520 && $(tt).length==1) {  $("div.submenu").hide(); $(tt).show(); refreshLayer(); return true; }
	if( $(tt).length==1 ) {
...
function refreshLayer() {
        ...
        hreflink(activediv);
		if(activediv == '#galeria' && mapa.getZoom() >= 13) {
...
else if(activediv == '#objekty' && mapa.getZoom() >= 13) {
...
else if(activediv == '#pocasie' && mapa.getZoom() >= 10) {
...
else if(activediv == '#trasy' && mapa.getZoom() >= 13) {
vrstva=$("input.leaflet-control-layers-selector:checked").parent().children("span").html().trim();
...
else { routeLayer.clearLayers(); $('.zoznam').text(' ');}
